...
class NewPlaylist with ChangeNotifier {
  ...
  final _onNewPlaylistController = StreamController.broadcast();
  ...
  void setTracksWithActiveTrack(List<Track?>? tracks, int index) {
    ...
    // launch events
    _onNewPlaylistController.add(true);
  ...
  Stream get onNewPlaylist => _onNewPlaylistController.stream;
  ...
  Track? nextInQueue() {
    if (canNext() == false) return null;
    return _tracks[_ids[_loop.getNextIndex()]];
  }
